...
//        let arrangement = [["crow_mask", "face"], ["blue_sunglasses", "face"], ["smoky_makeup", "face"]]
    ...
    //1. 종류및 종류의 개수를 가져온다.
    //2. 종류의 개수에 1을 더한다음 종류만큼 곱해준다. (옷을 안입은 경우 추가)
    //3. 2번 값에서 -1 을 한다. (모든 옷을 안입은 경우 삭제)
    func solution(_ clothes:[[String]]) -> Int {
        let clothesDic = getKindArray(clothes)
        var count = 1
    ...
    func getKindArray(_ clothes:[[String]]) -> [String : Int] {
        var valueDic : [String : Int] = Dictionary()
        for item in clothes{
            let value = valueDic[item[1], default: 0]
            if value == 0 {
                valueDic[item[1]] = 1
            } else {
                valueDic[item[1]] = value+1
            }
        }
        return valueDic
    }
